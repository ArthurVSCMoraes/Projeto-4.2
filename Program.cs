...
do
{
    ...
    switch (menuSelection)
    {
        ...
        case "2":
            ...
            string[] topicsSearch = dogCharacteristic.Split(",");
            for (int i = 0; i < topicsSearch.Length; i++)
            {
                topicsSearch[i] = topicsSearch[i].Trim();
            }
            Array.Sort(topicsSearch);
            ...
            for (int i = 0; i < maxPets; i++)
            {
                if (ourAnimals[i, 1].Contains("dog"))
                {
                    ...
                    bool matchesDogs = false;
                    foreach (var topic in topicsSearch)
                        if (topic != null && topic.Trim() != "")
                        {
                            ...
                            if (dogDescription.Contains("" + topic.Trim() + ""))
                            {
                                // #3b update message to reflect term 
                                // #3c set a flag "this dog" is a match
                                Console.WriteLine($"\nOur dog {ourAnimals[i, 3]} matches yout search for {topic.Trim()}!");
